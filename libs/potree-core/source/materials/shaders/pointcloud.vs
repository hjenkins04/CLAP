...
void main() {
	...
	// POINT COLOR SELECTION
			...
			vColor = vec4(w, w, w, 1.0);
		#elif defined color_type_height
			vColor = vec4(getElevation(), 1.0);
		#elif defined color_type_classification
			vColor = getClassification();
		#elif defined color_type_intensity_gradient
			float w = getIntensity();
			vColor = vec4(texture(gradient, vec2(w, 1.0 - w)).rgb, 1.0);
		#elif defined color_type_depth
			float linearDepth = -mvPosition.z;
			float expDepth = (gl_Position.z / gl_Position.w) * 0.5 + 0.5;
			vColor = vec4(linearDepth, expDepth, 0.0, 1.0);
		#elif defined color_type_color
			vColor = vec4(uColor, 1.0);
		#elif defined color_type_lod
			float wl = getLOD() / 10.0;
			vColor = vec4(texture(gradient, vec2(wl, 1.0 - wl)).rgb, 1.0);
		#elif defined color_type_point_index
			vColor = vec4(indices.rgb, 1.0);
		#elif defined color_type_return_number
			vColor = vec4(getReturnNumber(), 1.0);
		#elif defined color_type_source
			vColor = vec4(getSourceID(), 1.0);
		#elif defined color_type_normal
			vColor = vec4((modelMatrix * vec4(normal, 0.0)).xyz, 1.0);
		#else
			vColor = rgba;
		#endif
