 ...
 * The getZ callback returns the draw-plane Z for a given (x,y).
 */
export function buildShapeVisual(
  ...
  switch (shape.type) {
    case 'rect-2d': {
      ...
      if (!skipDots) {
        addDot(group, min.x, min.y, z00, color);
        addDot(group, max.x, min.y, z10, color);
        addDot(group, max.x, max.y, z11, color);
        addDot(group, min.x, max.y, z01, color);
      }
      break;
    }
    ...
    case 'box': {
      ...
      if (halfExtents.z > 0.01) {
        ...
        // Bottom face
        addLineLoop(group, [
          x0, y0, zBot, x1, y0, zBot, x1, y1, zBot, x0, y1, zBot, x0, y0, zBot,
        ], color);
        // Top face
        addLineLoop(group, [
          x0, y0, zTop, x1, y0, zTop, x1, y1, zTop, x0, y1, zTop, x0, y0, zTop,
        ], color);
        // 4 vertical edges
        addLineLoop(group, [x0, y0, zBot, x0, y0, zTop], color);
        addLineLoop(group, [x1, y0, zBot, x1, y0, zTop], color);
        addLineLoop(group, [x1, y1, zBot, x1, y1, zTop], color);
        addLineLoop(group, [x0, y1, zBot, x0, y1, zTop], color);
      } else {
        // Flat footprint
        const corners = [
          x0, y0, getZ(x0, y0),
    ...
    case 'cylinder': {
      ...
      if (height > 0.01) {
        // 3D wireframe cylinder: top ring, bottom ring, vertical lines
        ...
        for (let i = 0; i <= segments; i++) {
          ...
          botRing.push(px, py, zMin);
          topRing.push(px, py, zMax);
        }
        ...
        for (let i = 0; i < 4; i++) {
          ...
          const px = center.x + Math.cos(angle) * radius;
          const py = center.y + Math.sin(angle) * radius;
          addLineLoop(group, [px, py, zMin, px, py, zMax], color);
        }
      } else {
        ...
        for (let i = 0; i <= segments; i++) {
          ...
          const px = center.x + Math.cos(angle) * radius;
          const py = center.y + Math.sin(angle) * radius;
          positions.push(px, py, getZ(px, py));
        }
        addLineLoop(group, positions, color);
        if (!skipDots) {
          addDot(group, center.x, center.y, getZ(center.x, center.y), color);
        }
      }
      break;
    }
    ...
    case 'polygon-2d': {
      ...
      for (const v of verts) {
        positions.push(v.x, v.y, getZ(v.x, v.y));
      }
      if (verts.length >= 3 && !isPreview) {
        positions.push(verts[0].x, verts[0].y, getZ(verts[0].x, verts[0].y));
      }
      addLineLoop(group, positions, color);

      if (!skipDots) {
        for (const v of verts) {
          addDot(group, v.x, v.y, getZ(v.x, v.y), color);
        }
      }
      break;
    }
...
export function buildEditHandles(
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    mesh.position.set(p.x, p.y, p.z ?? getZ(p.x, p.y));
